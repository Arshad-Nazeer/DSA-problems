...
public:
    int recur(string &s, int i, int ans, int sign, bool started){
        ...
        if(!started && (s[i]=='-' || s[i]=='+')) return recur(s, i+1, ans, s[i]=='-' ? -1 : 1, true); //started set to true after first sign encountered so that all the conditions if conditions doeesnt corrupt the ans
